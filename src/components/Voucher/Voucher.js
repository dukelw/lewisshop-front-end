...
import axios from 'axios'; // Added missing imports
import styles from './Voucher.module.scss';
import Button from '../Button';
import { getAllShops, getDiscountsOfShopByUser, findProductsByID } from '~/redux/apiRequest';
...
function Voucher() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const shopsData = await getAllShops(accessToken, userID, dispatch, axiosJWT);
      setShops(shopsData?.metadata);
      const updatedVoucher = {};
      await Promise.all(
        shopsData?.metadata?.map(async (shop) => {
          const discounts = await getDiscountsOfShopByUser(accessToken, userID, shop._id, dispatch, axiosJWT);
          updatedVoucher[shop._id] = discounts?.metadata;
        }),
      );
        ...
        (shop, index) =>
          voucher[shop._id]?.length > 0 && (
            <div className={cx('shop')} key={index}>
              ...
              <Container>
                <Row>
                  {voucher[shop._id]?.map((discount, index) => (
